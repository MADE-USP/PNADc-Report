---
<<<<<<< HEAD
title: "Relatório Trimestral da PNAD^[Este relatório foi elaborado pelas pesquisadoras Clara Brenck e Eslen Brito e pelos pesquisadores Hiaman Santos e José Bergamin.]"
author: "Made USP"
date: "20/02/2024"
lang: pt-br
=======
title: "Relatório Trimestral da PNAD^[Este relatório foi elaborado pelas pesquisadoras Clara Brenck, Eslen Brito e pelos pesquisadores Hiaman Santos e José Bergamin.]"
author: "Made USP"
date: "20/02/2024"
...
load("C:\USER\Downloads\PNAD_dados_relatorio_04_2023.RData")
...
<<<<<<< HEAD
![Taxa de Ocupação por Raça e Gênero](C:/Users/clara/Downloads/ocupado_RG.png){width=90%}


![Taxa de Ocupação por Região](C:/Users/clara/Downloads/ocupado_REG.png){width=90%}
...
Inicialmente, destacamos a variação da taxa de desemprego geral de `r desem_tri*100` pontos percentuais em relação ao `r q-1`º trimestre de `r i`. A região Nordeste registrou maior desemprego ao fim do último trimestre (`r max_desemREG*100`%), enquanto o Sul teve o menor resultado no mesmo período (`r qui_desemREG*100`%). Nas categorias de raça e gênero, mulheres negras foram a maior população desempregada, cerca de `r max_desemRG*100`%. A menor taxa de desocupação foi registrada dentre homens brancos, de `r qua_desemRG*100`%. 
...
=======
Veja abaixo mais dados de desemprego:
...
<<<<<<< HEAD
Na comparação anual, a variação média do Brasil foi de `r (difano_informalRG)*100` pontos percentuais e a maior variação por raça e gênero foi registrada dentre homens brancos, uma diferença de `r max_infanoRG*100` pontos percentuais. Veja mais nos figuras 3 e 4.

![Taxa de Informalidade por Raça e Gênero](C:/Users/clara/Downloads/informal_RG.png){width=}


![Taxa de Informalidade por Região](C:/Users/clara/Downloads/informal_REG.png){width=}
...
A ocupação por setores varia consideravelmente de acordo com recortes de raça e gênero. No `r q`º trimestre de `r i`, a maioria dos homens negros (`r max_HN`%) estavam alocados no setor de SETOR, seguido de `r seg_HN` (VALOR%), `r ter_HN` (VALOR%), `r qua_HN` (VALOR%) e `r qui_HN` (VALOR%). Para homens brancos, o setor que mais empregou foi `r max_HB` com VALOR%, além de `r seg_HB` (VALOR%), `r ter_HB` (VALOR)%, `r qua_HB` (VALOR)% e `r qui_HB` (VALOR)%. 
...
![Rendimento Habitual por Raça e Gênero (reais de 2023)](C:/Users/clara/Downloads/RENDA_habit_RG.png){width=90%}
...
O rendimento habitual médio do trabalhador brasileiro foi de R$ `r renda_med` por mês no `r q`º trimestre de `r i`. O grupo populacional com maior aumento da renda média na comparação anual foi o de `r max_renhab_ano`, com VALOR, enquanto `r qua_renhab_ano` tiveram o menor aumento (VALOR). veja os dados no figura 5.


=======
O rendimento habitual médio do trabalhador brasileiro foi de R$ `r renda_med` por mês no `r q`º trimestre de `r i`. O grupo populacional com maior aumento da renda média na comparação anual foi o de `r max_renhab_ano`, com VALOR, enquanto `r qua_renhab_ano` tiveram o menor aumento (VALOR). veja os dados a abaixo:
>>>>>>> d652c9436dab38d2a95b598e7f8d757feee55249
...
<<<<<<< HEAD
![Índice de Gini por Região](C:/Users/clara/Downloads/gini_REG_2304.png){width=90%}
...
1998).
>>>>>>> 0f64a1e3dc93815008b1331e779cdf254e1c5cbe
>>>>>>> d652c9436dab38d2a95b598e7f8d757feee55249
