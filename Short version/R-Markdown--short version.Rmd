...
#Clara
load("C:/Users/lauro/Desktop/PNADc/v3/PNAD_dados_relatorio_01_2024.RData")
load("C:/Users/lauro/Desktop/PNADc/v3/dados_serie_historica_012024.RData")
...
agreInformal.RG <- agreInformal.RG %>% 
  select(Categoria, FormalidadeMedia.RG, InformalidadeMedia.RG)
...
# Voltamos para colunas originais
agreInformal.RG<- agreInformal.RG %>%
   select(Categoria, FormalidadeMedia.RG, InformalidadeMedia.RG, Data)
...
#Região
inf_NO <- agreInformal.REG$Informal[agreInformal.REG$Regiao == "Norte" & agreInformal.REG$Trimestre == "2024 Q1"]

inf_NE <- agreInformal.REG$Informal[agreInformal.REG$Regiao == "Nordeste" & agreInformal.REG$Trimestre == "2024 Q1"]

inf_CO <- agreInformal.REG$Informal[agreInformal.REG$Regiao == "Centro-Oeste" & agreInformal.REG$Trimestre == "2024 Q1"]

inf_SE <- agreInformal.REG$Informal[agreInformal.REG$Regiao == "Sudeste" & agreInformal.REG$Trimestre == "2024 Q1"]

inf_SU <- agreInformal.REG$Informal[agreInformal.REG$Regiao == "Sul" & agreInformal.REG$Trimestre == "2024 Q1"]
...
max_Inf_REG_2023_04 <- agreInformal.REG %>%
  ...
  filter(Informalidade.REG == max(Informalidade.REG)) %>% # e peço a linha correspondente ao valor máximo
...
min_INF_REG_2023_04 <- agreInformal.REG %>%
  filter(Trimestre == "2024 Q1") %>% #filtro pelo trimestre que quero 
  filter(Informalidade.REG == min(Informalidade.REG)) %>%  # e peço a linha correspondente ao valor mínimo
  select(Regiao) #peço a categoria associada
...
informal_HB <- agreInformal.RG$Informal[agreInformal.RG$Categoria == "Homem Branco" & agreInformal.RG$Data == "2024 Q1"]
informal_HN <- agreInformal.RG$Informal[agreInformal.RG$Categoria == "Homem Negro" & agreInformal.RG$Data == "2024 Q1"]
informal_MB <- agreInformal.RG$Informal[agreInformal.RG$Categoria == "Mulher Branca" & agreInformal.RG$Data == "2024 Q1"]
informal_MN <- agreInformal.RG$Informal[agreInformal.RG$Categoria == "Mulher Negra" & agreInformal.RG$Data == "2024 Q1"]
...
Neste relatório, reportamos a parcela das pessoas empregadas que são contribuintes para o sistema de previdência social (INSS). Esta medida pode ser entendida como uma proxy de informalidade^[Ver Kassouf, A. L. Wage gender discrimination and segmentation in the Brazilian labor market. Economia Aplicada, v. 2, n. 2, p. 243-269, 1998 e Dalberto, C. e Cirino, J. Informalidade e segmentação no mercado de trabalho brasileiro: evidências quantílicas sob alocação endógena. Nova Economia. v.28 n.2 p.417-460, 2018.], ainda que existam outras medidas possíveis -- como a parcela dos trabalhadores que possuem carteira assinada. A taxa de trabalhadores não contribuintes para o INSS média do Brasil ficou em `r informalidade*100`% e a região com maior taxa foi a região `r max_Inf_REG_2023_04$Regiao` (`r max_infREG*100`%), seguida do Nordeste (`r seg_infREG*100`%). A região `r min_INF_REG_2023_04$Regiao` registrou a menor taxa de trabalhadores não contribuintes (`r qui_infREG*100`%). 

No recorte de raça e gênero, o grupo com maior proporção de trabalhadores não contribuintes foi a categoria "`r max_Inf_RG_2023_04$Categoria`", com `r max_infRG*100`%. Em comparação com o trimestre anterior, a taxa geral de trabalhadores não contribuintes variou em `r dif_informalidade_tri*100` pontos percentuais e o grupo com maior aumento no mesmo período foi o de mulheres brancas, com uma diferença de `r max_inftriRG*100` pontos percentuais.

Na comparação anual, a variação média do Brasil foi de `r difano_informalRG*100` pontos percentuais e a maior variação por raça e gênero foi registrada dentre mulheres negras, uma diferença de `r qua_infanoRG*100` pontos percentuais. 

Veja o comportamento das séries de trabalhadores contribuintes para o INSS nas figuras 4 e 5.
