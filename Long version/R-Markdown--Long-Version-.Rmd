...
Regionalmente, o desemprego também apresentou tendência de queda no `r q`º trimestre de `r i`. A região Norte, registrou a maior taxa de desemprego de `r (max_desemRG)*100`% (queda de `r (desemanoNO)*100` pontos percentuais em um ano). A menor taxa de desemprego foi registrada na região Sul, `r (desemprego_SU)*100`%, com diferença de `r (desemanoSU)*100` p.p. em um ano. Na região Nordeste, a taxa de desemprego registrada foi de `r (desemprego_NO)*100`% (recuo de `r (desemano_NO)*100` p.p. em um ano). Na região Centro-Oeste, a taxa de desemprego medida foi de `r (desemprego_CO)*100`% (queda de `r (desemano_CO)*100` p.p. em um ano). Por fim, na região Sudeste, a taxa de desemprego registrada foi de `r (desemprego_SE)*100`% (recuo de `r (desemano_SE)*100` p.p. em um ano).
