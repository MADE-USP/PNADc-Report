...
#Eslen
#setwd("C:/Users/USER/Desktop/GitHub/.github/PNADc-Report/Input/Relatorio_PNAD_Continua")
...
Regionalmente, o desemprego também apresentou tendência de queda no `r q`º trimestre de `r i`. A região Norte, registrou a maior taxa de desemprego de `r (max_desemREG)*100`% (queda de `r (desemanoNO)*100` pontos percentuais em um ano). A menor taxa de desemprego foi registrada na região Sul, `r (desemprego_SU)*100`%, com diferença de `r (desemanoSU)*100` p.p. em um ano. Na região Nordeste, a taxa de desemprego registrada foi de `r (desemprego_NE)*100`% (recuo de `r (desemanoNE)*100` p.p. em um ano). Na região Centro-Oeste, a taxa de desemprego medida foi de `r (desemprego_CO)*100`% (queda de `r (desemanoCO)*100` p.p. em um ano). Por fim, na região Sudeste, a taxa de desemprego registrada foi de `r (desemprego_SE)*100`% (recuo de `r (desemanoSE)*100` p.p. em um ano).
...
#Região
escol_fund_NO <- ESCOLARIDADE.REG$ensino_fundamental[ESCOLARIDADE.REG$Categoria == "Norte"]
escol_fund_NE <- ESCOLARIDADE.REG$ensino_fundamental[ESCOLARIDADE.REG$Categoria == "Nordeste"]
escol_fund_CO <- ESCOLARIDADE.REG$ensino_fundamental[ESCOLARIDADE.REG$Categoria == "Centro-Oeste"]
escol_fund_SE <- ESCOLARIDADE.REG$ensino_fundamental[ESCOLARIDADE.REG$Categoria == "Sudeste"]
escol_fund_SU <- ESCOLARIDADE.REG$ensino_fundamental[ESCOLARIDADE.REG$Categoria == "Sul"]

escol_sup_NO <- ESCOLARIDADE.REG$ensino_superior[ESCOLARIDADE.REG$Categoria == "Norte"]
escol_sup_NE <- ESCOLARIDADE.REG$ensino_superior[ESCOLARIDADE.REG$Categoria == "Nordeste"]
escol_sup_CO <- ESCOLARIDADE.REG$ensino_superior[ESCOLARIDADE.REG$Categoria == "Centro-Oeste"]
escol_sup_SE <- ESCOLARIDADE.REG$ensino_superior[ESCOLARIDADE.REG$Categoria == "Sudeste"]
escol_sup_SU <- ESCOLARIDADE.REG$ensino_superior[ESCOLARIDADE.REG$Categoria == "Sul"]


sup_regs <- c(escol_sup_NO, escol_sup_NE, escol_sup_CO, escol_sup_SE, escol_sup_SU)
fund_regs <- c(escol_fund_NO, escol_fund_NE, escol_fund_CO, escol_fund_SE, escol_fund_SU)

max_escol_sup <- max(sup_regs)
max_escol_fund <- max(fund_regs)
...
giniRG.sh<- giniRG.sh %>%
   select(gini, Categoria, Trimestre)

```

```{r echo=FALSE}

ggplot(giniRG.sh, aes(x=Trimestre, y=gini, color=Categoria))+ 
  geom_line(size=2) + # definindo grossura da linha
...
colnames(trabalhosetor.RG.sh)[colnames(trabalhosetor.RG.sh) == "Regiao"] = "Região"
